    ...
    def __init__(self, device, params, **kwargs):
        ...
        for channel, channel_params in self.params.items():
            ...
            if charge_limit is not None:
                ...
                if (
                    not math.isfinite(charge_limit)
                    or charge_limit <= 0
                ):
                    raise ValueError(
                        "charge_limit_mAh must be a "
                        "positive finite number."
                    )

                if not self._record_ece:
                    raise ValueError(
                        "record_ece must be True when "
                        "using charge_limit_mAh."
                    )
    ...
    def set_charge_limit(
        ...
        for selected_channel in selected_channels:
            ...
            if charge_limit_mAh is None:
                ...
                if (
                    not math.isfinite(validated_limit)
                    or validated_limit <= 0
                ):
                    raise ValueError(
                        "charge_limit_mAh must be a "
                        "positive finite number."
                ...
                if not self._record_ece:
                    raise ValueError(
                        "record_ece must be True before "
